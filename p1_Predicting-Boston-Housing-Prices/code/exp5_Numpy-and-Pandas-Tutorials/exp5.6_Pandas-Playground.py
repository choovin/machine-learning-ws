...
1) dtypes: to get the datatype for each column 获取每一列的数据类型
2) describe: useful for seeing basic statistics of the dataframe's numerical
   columns 对于查看数据帧数字列的基本统计信息非常有用
3) head: displays the first five rows of the dataset 显示头五行的数据集
4) tail: displays the last five rows of the dataset 显示最后五行的数据集
'''
